...
namespace Karion.BusinessSolution.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : BusinessSolutionServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("Karion.BusinessSolution", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);

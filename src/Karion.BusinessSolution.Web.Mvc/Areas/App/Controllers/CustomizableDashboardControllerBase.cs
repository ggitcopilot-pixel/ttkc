...
using Karion.BusinessSolution.DashboardCustomization.Dto;
...
{
    public abstract class CustomizableDashboardControllerBase : BusinessSolutionControllerBase
    {
        ...
        public async Task<PartialViewResult> AddWidgetModal(string dashboardName, string pageId)
        {
            var userDashboard = await DashboardCustomizationAppService.GetUserDashboard(
                new GetDashboardInput
                {
                    ...
                    Application = BusinessSolutionDashboardCustomizationConsts.Applications.Mvc
                }
            );

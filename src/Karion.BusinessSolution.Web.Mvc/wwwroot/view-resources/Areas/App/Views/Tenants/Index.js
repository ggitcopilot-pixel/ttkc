(function () {
    $(function () {
        ...
        var _entityTypeFullName = 'Karion.BusinessSolution.MultiTenancy.Tenant';


        var _permissionsHanet = {
        ...
        var _createOrEditModalHanet = new app.ModalManager({
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (displayName, type, row, meta) {
                        var returns = "<table class='table table-hover table-striped table-bordered'>";
                        if(row.listPlace.length===0){
                            return returns+"<tr><td>Chua cap nhat place</td></tr><tr><td><a class='btn btn-success btn-add-places' data-target='"+row.id+"' style='color:white;display:block'><i class='fa fa-plus'></i> Thêm mới places</a></td></tr></table>";
                        }
                        returns+="<td>Tên</td><td>Địa chỉ</td><td>Thiết bị</td>";
                        $.each(row.listPlace,function (index, value) {
                            var tables = "<table class='table table-hover table-striped table-bordered'>";
                            $.each(value.devices,function (indexdevice,valuedevice) {
                                tables+="<tr><td>"+valuedevice.deviceName+"</td><td><i class='fa fa-dot-circle' title='Last Check: "+moment(valuedevice.lastCheck).format('L LTS')+"' style='color:"+((valuedevice.deviceStatus)?"green":"red")+"'></i></td></tr>";
                            });
                            tables+="</table>";
                            returns+="<tr style='border-top:2px solid black'><td>"+value.placeName+"</td><td>"+value.placeAddress+"</td><td>"+tables+"<a style='display: block;color: white' class='btn btn-warning btn-update-device' data-target='"+value.id+"'><i class='fa fa-refresh'></i> Cập nhật thiết bị</a></td></tr>"
                        });
                        return returns+"<tr><td colspan='3'><a class='btn btn-success btn-add-places' data-target='"+row.id+"' style='color:white;display:block'><i class='fa fa-plus'></i> Thêm mới places</a></td></tr></table>";
                    }
                }
            ]
        });
        ...
        $(document).ready(function () {
           $(document).on('click','.btn-add-places',function () {
                var self = $(this);
                if(_permissionsHanet.create){
                    _createOrEditModalHanet.open({ tenantid: self.attr("data-target")}).done()
                }else{
                    abp.message.warn("Bạn không có quyền cập nhật");
                }
           });
           $(document).on('click','.btn-update-device',function () {
                var self = $(this);
               abp.message.confirm(
                   app.localize("Cập nhật thiết bị?"),
                   app.localize('AreYouSure'),
                   function (isConfirmed) {
                       if (isConfirmed) {
                           if(_permissionsHanet.create){
                               _hanetService.updateDevices(self.attr("data-target")).done(function () {
                                   getTenants();
                               });
                           }else{
                               abp.message.warn("Bạn không có quyền cập nhật");
           ...
           $(document).on('click','#reloadstatus',function () {
                var self = $(this);
               abp.message.confirm(
                   app.localize("Cập nhật trạng thái thiết bị?"),
                   app.localize('AreYouSure'),
                   function (isConfirmed) {
                       if (isConfirmed) {
                           if(_permissionsHanet.create){
                               _hanetService.updateDevicesStatus().done(function () {
                                   getTenants();
                               });
                           }else{
                               abp.message.warn("Bạn không có quyền cập nhật");
        ...
        abp.event.on("app.createOrEditHanetTenantPlaceDatasModalSaved",
            function () {
                getTenants(true);
            });
